# Minecraft 1.20.6 Fabric Injection Principle

[English](1.20.6+fabric.md)

## Overview

MCP Mod for Minecraft 1.20.6 Fabric uses **SpongePowered Mixin** bytecode injection to hook into the vanilla Minecraft client. This version uses the `MouseClickMixin` pattern (introduced in 1.19.4) which targets `ParentElement.mouseClicked()` instead of `Mouse.onMouseButton()`.
...
This is an **interface mixin** targeting `ParentElement` (which all `Screen` implementations extend). This architecture is elegant because:
...
- **Java 21**: Minecraft 1.20.6 ships with Java 21, reflected in the `JAVA_21` compatibility level
- **Fabric Loader 0.16.0**: Uses the 0.16.x loader series with updated Loom toolchain
- **ParentElement interface mixin**: The `MouseClickMixin` (introduced in 1.19.4) operates on `ParentElement` — all Screen instances inherit this interface in the official mappings
- **Official mappings**: 1.20.6 uses Mojang's official mappings (`net.minecraft.*`) rather than intermediary names
- **Chat component API**: Uses `Component.translatable()` and `ClickEvent.Action.OPEN_URL` (the modern API introduced in 1.19)
- **1.19.4**: Fabric Loader 0.15.6, Java 17 compile, first version with `MouseClickMixin`
...
| `src/main/resources/mcpmod.mixins.json` | Mixin configuration (JAVA_21) |
