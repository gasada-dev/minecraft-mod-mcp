...
- **1.13.2**: Forge 25.0.223, Java 8. First GLFW-based version (transitional). Simpler mouse handling — relies on event cancellation, not GLFW callback interception.
...
- **1.17.1**: Forge 37.1.1, Java 16. Requires Java 16+ (significant JVM requirement change). Depth-based rendering changes begin. Minecraft starts using more modern Java features (records, pattern matching preview). Reflective field access may differ due to Java 16 strong encapsulation.
