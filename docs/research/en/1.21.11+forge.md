# Minecraft 1.21.11 Forge Injection Principle

[English](1.21.11+forge.md)

## Overview

MCP Mod for Minecraft 1.21.11 Forge uses the **Forge Event Bus** with the **new static BUS API**. This is the newest Forge era (ForgeGradle 7+) which introduces static event bus references (e.g., `ScreenEvent.Opening.BUS`) replacing the deprecated `FMLJavaModLoadingContext` approach. Forge 61.x splits tick events into `Pre`/`Post` inner classes (`TickEvent.ClientTickEvent.Post.BUS`), and `Window.getWindow()` was renamed to `Window.handle()`.
...
flowchart TD
    subgraph "Forge Static BUS API - 1.21.7"
        ...
        CTR --> TC[TickEvent.ClientTickEvent.Post.BUS]
    end
...
Forge 61.1.5, ForgeGradle 7.0.23+, Java 21. Uses official Mojang mappings. The static BUS API is the defining feature of this version. Forge 61.x splits `TickEvent.ClientTickEvent` into `Pre` and `Post` inner classes, removing the need for `event.phase` checks. `Window.getWindow()` renamed to `Window.handle()`. `ScreenEvent.Opening` uses `event.getNewScreen()` and `event.setNewScreen(null)` instead of `getScreen()`/`setCanceled()`. `InputEvent.MouseButton.Pre.BUS` listeners return boolean (true = cancel). `ClickEvent.OpenUrl` requires a `java.net.URI` object. 298 lines in ModDevMcpMod.

## Known Limitations

### Left-click blocked in MCP control mode

While in MCP control mode, all left-clicks (button 0) are consumed by the GLFW interceptor and never forwarded to Minecraft's original callback. This means attack, block breaking, and other left-click actions do not work during MCP control. Right-click is unaffected and works normally.

**Cause**: 1.21.11 Forge's static `BUS` API causes dual event handling with the GLFW callback. If non-overlay left-clicks are forwarded to MC's original callback, the cursor teleports to the center and triggers unpredictable state transitions (e.g., auto-exiting MCP then re-entering). To ensure overlay button clicks (resume/menu) remain stable, the current implementation fully blocks left-clicks during MCP control. This issue is consistent across both Forge and NeoForge for 1.21.11.
...
| Feature | Old Forge (1.20.6) | New Forge (1.21.11+) |
|---------|-------------------|---------------------|
...
| `src/main/java/.../ModDevMcpMod.java` | Main mod class (~280-298 lines) |
