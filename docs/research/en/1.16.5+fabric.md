# Minecraft 1.16.5 Fabric Injection Principle

[English](1.16.5+fabric.md)

## Overview

MCP Mod for Minecraft 1.16.5 Fabric uses **SpongePowered Mixin** bytecode injection to hook into the vanilla Minecraft client rendering loop and input handling. The Mixin framework applies compile-time bytecode weaving via the Fabric Loader, redirecting method calls through annotated mixin classes.
...
- **Java 8** (but Java 16 compile): Version 1.16.5 uses `JAVA_8` compatibility level with Java 16 compilation
