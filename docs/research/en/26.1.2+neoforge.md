# Minecraft 26.1.2 NeoForge Injection Principle

[English](26.1.2+neoforge.md)

## Overview

MCP Mod for Minecraft 26.1.2 NeoForge uses the **NeoForge Event Bus** system with **pure event-driven injection** — no Mixins, no GLFW callback hijacking. Unlike Forge, NeoForge relies entirely on event cancellation through its `IEventBus` dependency injection pattern. This makes the injection cleaner but means mouse control is limited to what events can cancel.
...
- **NeoForge 26.1.2.36-beta**, Java 25, NeoGradle 2.0.141
- Uses **`GuiGraphicsExtractor`** instead of `GuiGraphics` (same as Forge 26.1.2)
- Font rendering uses reflective `drawInBatch()` due to API changes with non-official mappings
- Chat messages sent via reflective `addMessage()` — searches for method by name and checks parameter type compatibility (fallback: prints to stdout)
- Separate `registerInputInterception()` method for input handling organization
- Separate `tick()` method encapsulates tick logic for reuse in both HUD and screen contexts
- 207 lines — still shorter than the Forge equivalent (280 lines)
- Uses `ClickEvent.OpenUrl(URI)` (new API in 1.21+)

## Key Differences: NeoForge vs Forge Summary

| Feature | Forge 1.20.6 | NeoForge 26.1.2 |
|---------|-------------|------------------------|
...
| Rendering class | `GuiGraphics` (1.20.6) / `GuiGraphicsExtractor` (26.1.2) | `GuiGraphicsExtractor` |
| Keyboard intercept | Not used | Not implemented (no `InputEvent.Key.BUS` equivalent used) |
| Chat send | Direct `mc.gui.getChat().addMessage()` | Reflective `addMessage()` call with argument matching (non-official mappings) |
